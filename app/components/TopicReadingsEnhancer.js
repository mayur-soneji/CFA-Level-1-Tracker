...
function ReadingPanel({title,topicId,readings}){
 ...
 return <div className="topicReadings">
  <button type="button" className="topicReadingsToggle" aria-expanded={open} onClick={()=>setOpen(v=>!v)}>
   <span className="topicReadingsToggleText"><span className="topicReadingsLabel">READING TRACKER</span><span>{completed.length}/{readings.length} readings · {percent}%</span></span><span className="topicReadingsChevron" aria-hidden="true">{open?'−':'+'}</span>
  </button>
  <div className="topicReadingProgressMeta" aria-label={`${completed.length} of ${readings.length} readings complete. ${completedHours.toFixed(1)} of ${plannedHours} planned hours covered.`}><span className="topicReadingProgressTrack"><span style={{width:`${percent}%`}}/></span><span className="topicReadingProgressTime">{completedHours.toFixed(1)}h / {plannedHours}h planned</span></div>
  {open&&<div className="topicReadingsBody">
   <div className="topicReadingsHead"><span>{title}</span><span>{hoursPerReading.toFixed(1)}h est. / reading</span></div>
...
export default function TopicReadingsEnhancer(){
 ...
 useEffect(()=>{const findTargets=()=>{const next=Array.from(document.querySelectorAll('.topicGrid .topic')).map(node=>{let target=node.querySelector(':scope > .topicReadingsMount');if(!target){target=document.createElement('div');target.className='topicReadingsMount';node.appendChild(target)}const title=node.querySelector('.topicTitle')?.textContent?.trim()||'';return{target,title,topicId:TOPIC_IDS[title]}}).filter(x=>x.topicId);setTargets(next)};findTargets();const observer=new MutationObserver(findTargets);observer.observe(document.body,{childList:true,subtree:true});return()=>observer.disconnect()},[]);
