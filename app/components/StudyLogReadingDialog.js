...
function saveAttemptAndSession({date,hours,topicId,reading,status}){try{
 ...
 logs.unshift({id:`${Date.now()}-${Math.random()}`,date,hours,topic:topicId,focus:`Reading ${String(reading.number).padStart(2,'0')} — ${reading.title} — ${status==='completed'?'Completed':'Incomplete'}`,readingNumber:reading.number,readingTitle:reading.title,readingStatus:status});
 ...
 window.dispatchEvent(new CustomEvent('cfa-study-log-added',{detail:{topicId,readingNumber:reading.number,status,date,hours}}));
 return {ok:true};
